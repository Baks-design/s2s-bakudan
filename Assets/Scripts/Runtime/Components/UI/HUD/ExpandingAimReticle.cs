...
namespace Game.Runtime.Components.UI
{
    ...
    public class ExpandingAimReticle : MonoBehaviour
    {
        ...
        [SerializeField, Vector2AsRange] Vector2 RadiusRange;
        [Tooltip("The time is takes for the aim reticle to adjust, when inaccurate.")]
        [SerializeField, Range(0f, 1f)] float BlendTime;
        ...
        [SerializeField] RectTransform AimTargetReticle;
        float m_BlendVelocity;
        float m_CurrentRadius;

        void OnValidate()
        {
            RadiusRange.x = Mathf.Clamp(RadiusRange.x, 0f, 100f);
            RadiusRange.y = Mathf.Clamp(RadiusRange.y, RadiusRange.x, 100f);
        }
